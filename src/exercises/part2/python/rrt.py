...
def sample_random_position(occupancy_grid):
  ...
  if position[X] > 0 and position[X] < 0.6:
    if position[Y] > 0.15 and position[Y] < 0.45:
      print("LOOK AT THIS =================================")
...
def step_along(occupancy_grid, start, end):
  centre, r = find_circle(start, end)
  start, end = start.pose, end.pose
  angle_b = angle_between(start[:2] - centre, end[:2] - centre)
  clockwise = True if start[YAW] > end[YAW] else False
  va = start[:2] - centre


  for angle in np.linspace(0, angle_b, 100):
    if clockwise:
      angle = -angle
    rotated = rotate(va, angle) + centre
    if not occupancy_grid.is_free(rotated):
      return True
...
def adjust_pose(node, final_position, occupancy_grid):
  ...
  direction = final_position - node.pose[:2]
  angle = np.arctan2(direction[Y], direction[X])
  final_node.pose[YAW] = 2*angle - node.pose[YAW]
