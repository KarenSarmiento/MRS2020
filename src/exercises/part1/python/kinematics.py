...
def euler(current_pose, t, dt):
  ...
  next_pose[X] += dt*(u * np.cos(current_pose[YAW]))
  next_pose[Y] += dt*(u * np.sin(current_pose[YAW]))
  next_pose[YAW] += dt*(w)
...
def rk4(current_pose, t, dt):
  ...
  k1_yaw = w * dt
  k2_yaw = np.cos(t + (dt*0.5)) * dt 
  k3_yaw = np.cos(t + (dt*0.5)) * dt 
  k4_yaw = np.cos(t+dt) * dt
  next_pose[YAW] += (k1_yaw + 2*k2_yaw + 2*k3_yaw + k4_yaw) / 6

  k1_X = dt*(u * np.cos(current_pose[YAW]))
  k2_X = dt*(u * np.cos(current_pose[YAW] + 0.5 * k1_yaw))
  k3_X = dt*(u * np.cos(current_pose[YAW] + 0.5 * k2_yaw))
  k4_X = dt*(u * np.cos(current_pose[YAW] + k3_yaw))
  next_pose[X] += (k1_X + 2*k2_X + 2*k3_X + k4_X) / 6

  k1_Y = dt*(u * np.sin(current_pose[YAW]))
  k2_Y = dt*(u * np.sin(current_pose[YAW] + 0.5 * k1_yaw))
  k3_Y = dt*(u * np.sin(current_pose[YAW] + 0.5 * k2_yaw))
  k4_Y = dt*(u * np.sin(current_pose[YAW] + k3_yaw))
  next_pose[Y] += (k1_Y + 2*k2_Y + 2*k3_Y + k4_Y) / 6
