...
def braitenberg(front, front_left, front_right, left, right):
  ...
  ds = np.array([[1 / left], [1 / front_left], [1 / front], [1 / front_right], [1 / right]])
 
  # Weights
  ws = np.array([[0.2, 0, 0, 0.1, 0.2],
                 [-0.4, -1.5, -0.4, 1.5, 0.4]])
  ...
  # vl_weights = np.array([0.6, 1.55, 0.4, -1.35, -0.2])
  # vr_weights = np.array([-0.2, -1.55, -0.4, 1.55, 0.6])
...
class Particle(object):
  ...
  def __init__(self):
    ...
    self._pose[X] = np.random.uniform(-WALL_OFFSET, WALL_OFFSET, 1)[0]
    self._pose[Y] = np.random.uniform(-WALL_OFFSET, WALL_OFFSET, 1)[0]
    self._pose[YAW] = np.random.uniform(0, 2*np.pi, 1)[0]
  ...
  def is_valid(self):
    ...
    edges = WALL_OFFSET - ROBOT_RADIUS

    if self._pose[0] < -edges or self._pose[0] > edges:
      return False
    if self._pose[1] < -edges or self._pose[1] > edges:
      return False
    if np.linalg.norm(self._pose[0:1]-CYLINDER_POSITION) <= CYLINDER_RADIUS:
      return False
  ...
  def move(self, delta_pose):
    ...
    if random.random() > 0.075:
      change = np.array([np.random.normal(delta_pose[X], np.abs(0.1 * delta_pose[X])+0.0001, 1)[0],
                        0,
                        np.random.normal(delta_pose[YAW], np.abs(0.1 * delta_pose[YAW])+0.0001, 1)[0]])
  
      # Get global coordinates
      global_change = np.array([change[X] * np.cos(self._pose[YAW]), change[X] * np.sin(self._pose[YAW]), change[YAW]])
  
      # Update pose
      self._pose[X] += global_change[X]
      self._pose[Y] += global_change[Y]
      self._pose[YAW] += global_change[YAW]
    else:
      self.__init__()
  ...
  def compute_weight(self, front, front_left, front_right, left, right):
    ...
    def prob(angle, rob_dist):
      part_distance = min(self.ray_trace(angle),4)
      part_distribution = norm(part_distance, sigma)
      return part_distribution.pdf(rob_dist)

    p_front = prob(0, front)
    p_front_left = prob(np.pi / 4, front_left)
    p_front_right = prob(-np.pi / 4, front_right)
    p_left = prob(np.pi / 2, left)
    p_right = prob(-np.pi / 2, right)

    # p_front = norm(self.ray_trace(0), sigma).pdf(front)
    # p_front_left = norm(self.ray_trace(np.pi / 4), sigma).pdf(front_left)
    # p_front_right = norm(self.ray_trace(-np.pi / 4), sigma).pdf(front_right)
    # p_right = norm(self.ray_trace(-np.pi / 2), sigma).pdf(front)
    # p_left = norm(self.ray_trace(np.pi / 2), sigma).pdf(right)
 
    self._weight = (p_front + p_front_left + p_front_right + p_right + p_left)/5
